...
for (const [range, filter] of Object.entries(targets)) {
  ...
  if (ratios.length > 0) {
    ...
    compressionThresholds[range] = {
      p30: ratios[Math.floor(ratios.length * 0.30)],
      p40: ratios[Math.floor(ratios.length * 0.40)],
    };
  }
}
...
// 5가지 조건 실험
const experiments = {
  'A1': { name: '기존 (>= 0.89)', test: (r) => r >= 0.89 },
  'A4': { name: '시총별 p30', test: null }, // range별로 다름
  'A5': { name: '시총별 p40', test: null }, // range별로 다름
};
...
const testSmallCsb = (conditionTest, rangeLabel) => {
  ...
  for (const [range, filter] of Object.entries(targets)) {
    ...
    for (const meta of rangeStocks) {
      ...
      const condCount = 1 + 1 + 1 + (breakoutReady ? 1 : 0); // support + volume + compression + breakout
...
console.log('= A4 (시총별 p30) =');
const a4 = testSmallCsb((r, range) => r <= compressionThresholds[range]?.p30);
console.log(`READY: ${a4.ready.total}`);
console.log(`WATCH: ${a4.watch.total}`);
console.log(`  500억~1000억: ${a4.watch.byRange['cap500to1000']}`);
console.log(`  1000억~2000억: ${a4.watch.byRange['cap1000to2000']}`);
console.log(`  2000억~3000억: ${a4.watch.byRange['cap2000to3000']}\n`);

console.log('= A5 (시총별 p40) =');
const a5 = testSmallCsb((r, range) => r <= compressionThresholds[range]?.p40);
console.log(`READY: ${a5.ready.total}`);
console.log(`WATCH: ${a5.watch.total}`);
console.log(`  500억~1000억: ${a5.watch.byRange['cap500to1000']}`);
console.log(`  1000억~2000억: ${a5.watch.byRange['cap1000to2000']}`);
console.log(`  2000억~3000억: ${a5.watch.byRange['cap2000to3000']}\n`);
...
for (const [range, vals] of Object.entries(compressionThresholds)) {
  ...
  console.log(`${rangeLabel}: p30=${vals.p30.toFixed(4)}, p40=${vals.p40.toFixed(4)}`);
}
