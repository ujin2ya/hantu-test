...
<body>
  ...
  <div class="nav">
    ...
    <a href="/qva-review-ok" style="color:#6ee7b7;border:1px solid #10b981;background:#0f172a;">⭐ 3단계 코호트 비교</a>
  </div>
  ...
  <div class="info-box">
    ...
    <p>H그룹(돌파 성공 확인 종목)에 진입한 뒤, 어떤 익절/청산 규칙이 가장 좋은 결과를 냈는가? 단순 보유 vs 부분/전량 익절 vs Trailing stop 중 무엇을 써야 하나?</p>
    ...
    <p style="font-size:12px;background:#0f172a;padding:8px 12px;border-radius:6px;border:1px solid #334155;">
      <span style="color:#a5b4fc;font-weight:700;">진입(3단계)</span> 후 <span style="color:#f9a8d4;font-weight:700;">청산 관리(4단계)</span> 검증. 진입까지의 알파를 어떻게 보존/회수할지를 정량 비교합니다.
    </p>
    ...
    <ul style="margin:4px 0;padding-left:20px;font-size:13px;line-height:1.7;color:#cbd5e1;">
      <li><strong>14개 시나리오 비교</strong> — HOLD_D10 / TP10/15/20/30 (전량) / TP10/15/20 절반 / TRAIL_FROM_HIGH_5/7 / STOP_VVI_LOW + TP15 절반 / STOP_-7% + TP15 절반 / TP15 절반 + Trailing 7 등</li>
      <li><strong>신호 기준</strong> = QVA 신호 건수 기준 (같은 종목의 같은 VVI가 여러 번 포함 가능)</li>
      <li><strong>이벤트 기준</strong> = (code, vviDate, entryDate) 중복 제거. <strong>최종 판단은 이벤트 기준으로 보세요.</strong></li>
      <li><strong>H그룹</strong> = 돌파 성공 확인 종목 / <strong>E그룹</strong> = 1% 돌파 발생만 (참고용)</li>
    </ul>

    <p style="margin-top:10px;"><strong>🎯 핵심 의미</strong></p>
    <p>진입 자체보다 청산 규칙이 결과를 어떻게 바꾸는지를 보여줍니다. 단순 10일 보유 vs +20% 익절 vs Trailing stop 등의 차이를 비교해 운영 규칙을 정합니다.</p>

    <p style="margin-top:10px;color:#fbbf24;">⚠️ 매수 추천이 아니라 조건 조합의 성과 검증입니다.</p>
  </div>
