...
<body>
  ...
  <div class="info-box">
    ...
    <p>QVA → VVI 후 다음 거래일에 VVI 고가 + 1% 돌파가 발생했을 때 실제 진입 후보로 볼 수 있는가? 진입 후 어떤 손절 조건(A~E)이 가장 효과적인가?</p>
    ...
    <p style="font-size:12px;background:#0f172a;padding:8px 12px;border-radius:6px;border:1px solid #334155;">
      <span style="color:#93c5fd;font-weight:700;">VVI(2단계)</span> → <span style="color:#a5b4fc;font-weight:700;">돌파 진입(3단계)</span> 조건 검증. <strong>H그룹 = 돌파 성공 확인 종목</strong>은 매일 운영 보드의 'BREAKOUT_SUCCESS' 카드와 같은 의미입니다.
    </p>
    ...
    <ul style="margin:4px 0;padding-left:20px;font-size:13px;line-height:1.7;color:#cbd5e1;">
      <li>그룹별로 진입 트리거 수, 5/10일 수익률, MFE10/MAE10, 5/10일 뒤 <strong>플러스 마감 비율</strong> 비교</li>
      <li>F/G 행은 손절 시나리오(A: VVI 저가 / B: VVI 종가 / C: -5% / D: -7% / E: 다음날 종가 &lt; vviHigh)를 적용한 조정 결과</li>
      <li><strong>중복 제거 비교</strong> 섹션 — code + vviDate + entryDate 기준 dedup. 신뢰도 더 높음.</li>
    </ul>

    <p style="margin-top:10px;"><strong>🎯 핵심 의미</strong></p>
    <p>1년 검증에서 <strong>H그룹의 20일 뒤 플러스 마감 비율은 71.0%, 평균 수익률 +15.1%</strong>로 QVA 단독(56.2%) 대비 큰 폭 개선됐습니다. 즉 funnel을 끝까지 통과한 후보가 실제 알파의 핵심임을 정량 확인.</p>
