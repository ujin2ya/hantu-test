...
| `POST /admin/refresh-all-boards` | adminController.postRefreshAllBoards | **전체 보드 갱신** — `adminTriggers.BOARD_SCRIPTS` 8개를 백그라운드 순차 실행 (QVA + QVA 고점 재돌파 + D+5 재돌파 운용 + QVA2 × 3 + 1DS + QVA 장중 감시). cron 16:35와 동일한 sequence를 admin에서 트리거. `patternState.refreshingAllBoards` / `allBoardsCurrent` / `allBoardsResults`로 진행 추적 |
...
| 16:35 | 평일 (월-금) | **전체 보드 갱신** — `BOARD_SCRIPTS` 8개 순차 실행: `boards/qva/qva-watchlist-board.js` → `boards/qva/qva-vvi-redefined-board.js` → `boards/rebreak/hgroup-rebreak-operation-board.js` → `boards/qva2/qva2-watchlist-board.js` → `boards/qva2/qva2-d5-rebreak-board.js` → `boards/qva2/qva2-vvi-board.js` → `boards/oneDaySurge/one-day-surge-board.js` → `boards/qva/qva-live-watch-board.js`. 차트/수급 캐시는 16:20 일일 업데이트에서 이미 갱신됨 — 보드는 캐시 read만. QVA 장중 감시 보드는 board_signals DB가 다른 QVA/QVA2 보드 실행 후에 최신 신호를 가져야 하므로 마지막에 배치 | 항상 (보드별 실패는 다음 보드로 진행) |
...
| 09:35 / 12:30 / 15:30 | 평일 (월-금) | **QVA 장중 감시 보드 분봉 수집 + 재생성** ([pipeline/refresh-qva-live-watch.js](pipeline/refresh-qva-live-watch.js)) — 3 phase: (1) qva-live-watch-board.js 1차로 DB에서 후보 코드(~416종) 추출 → (2) `collect-1ds-intraday.js --target-date {today} --codes ... --end-hour HHMMSS` 로 single endHour 분봉 수집 (각 cron이 적절한 endHour 전달: 09:35=100000 / 12:30=130000 / 15:30=153000) → (3) qva-live-watch-board.js 재실행. 후보 ~416종 × 1 call × 350ms ≈ 2.4분/run. 3 runs/day ≈ 7분 collect time. collector 가 endHour에서 windowTo/skip threshold 자동 추출 + 기존 파일과 merge 해서 cumulative bars 유지. KIS가 120 bars 만 반환하므로 09:35-10:30 / 12:30-13:30 시간대 bars는 갭으로 남을 수 있음. lock file (`reports/.qva-live-watch.lock`) 로 동시 실행 차단. 16:35 BOARD_SCRIPTS 의 qva-live-watch 는 별도 (collect 없이 보드만 — 15:30 cron 결과 사용) | 항상 |
