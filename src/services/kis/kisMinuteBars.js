...
const { getAccessToken, isExpiredTokenError } = require("./kisToken");

// KIS가 만료 토큰에 HTTP 500 + EGW00123을 주는데, 로컬 캐시는 아직 유효하다고 믿어
// 죽은 토큰을 계속 쓰는 경우가 있다. 만료 에러를 만나면 강제 재발급 후 1회 재시도한다.
// (token을 인자로 받는 모든 호출부가 이 자가복구의 혜택을 받음)
async function withTokenRetry(accessToken, doCall) {
  try {
...
async function getMinuteBarsAt(accessToken, stockCode, hourHHMMSS, includePastBusinessDay = "N") {
  ...
  const res = await withTokenRetry(accessToken, (tok) => axios.get(url, {
    headers: {
