...
app.get("/hgroup-rebreak-deep-dive", (req, res) => {
  const filePath = path.join(__dirname, "reports", "hgroup-rebreak-deep-dive-result.html");
  if (!fs.existsSync(filePath)) {
...
app.get("/d5-rebreak/:code", async (req, res) => {
  try {
    ...
    // 1) 재돌파 보드 JSON에서 항목 찾기
    const rebPath = path.join(__dirname, "reports", "hgroup-rebreak-operation-board-result.json");
    if (!fs.existsSync(rebPath)) {
    ...
    const rebData = JSON.parse(fs.readFileSync(rebPath, "utf-8"));
    ...
    // 2) 차트 데이터 (최근 60일)
    const chartPath = path.join(__dirname, "cache", "stock-charts-long", `${code}.json`);
    let chartRows = [];
