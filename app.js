...
app.get("/bms-qva-overlap", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-qva-overlap-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-qva-overlap-audit-result.html 파일이 없습니다. `node bms-qva-overlap-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── BMS Long-Base Audit ───────────
app.get("/bms-long-base-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-long-base-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-long-base-audit-result.html 파일이 없습니다. `node bms-long-base-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── BMS Value-Ratio Bucket Audit ───────────
app.get("/bms-value-ratio-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-value-ratio-bucket-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-value-ratio-bucket-audit-result.html 파일이 없습니다. `node bms-value-ratio-bucket-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── BMS Price-Position Bucket Audit ───────────
app.get("/bms-price-position-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-price-position-bucket-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-price-position-bucket-audit-result.html 파일이 없습니다. `node bms-price-position-bucket-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── BMS Core Condition Overlap Audit ───────────
app.get("/bms-core-condition-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-core-condition-overlap-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-core-condition-overlap-audit-result.html 파일이 없습니다. `node bms-core-condition-overlap-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── BMS Simple Score Audit ───────────
app.get("/bms-simple-score-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "bms-simple-score-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/bms-simple-score-audit-result.html 파일이 없습니다. `node bms-simple-score-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── VPR H그룹 눌림 재돌파 감사 보고서 ───────────
app.get("/vpr-hgroup-audit", (req, res) => {
  const filePath = path.join(__dirname, "reports", "vpr-hgroup-pullback-rebound-audit-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/vpr-hgroup-pullback-rebound-audit-result.html 파일이 없습니다. `node vpr-hgroup-pullback-rebound-audit.js`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── VPR H그룹 일별 보드 다음날 백테스트 ───────────
app.get("/vpr-hgroup-daily-backtest", (req, res) => {
  const filePath = path.join(__dirname, "reports", "vpr-hgroup-daily-board-nextday-backtest-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/vpr-hgroup-daily-board-nextday-backtest-result.html 파일이 없습니다. `node vpr-hgroup-daily-board-nextday-backtest.js --from=20260401 --to=20260430`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});

// ─────────── VPR H그룹 장기 성숙 백테스트 ───────────
app.get("/vpr-hgroup-long-backtest", (req, res) => {
  const filePath = path.join(__dirname, "reports", "vpr-hgroup-long-period-maturity-backtest-result.html");
  if (!fs.existsSync(filePath)) {
    return res.status(404).send("reports/vpr-hgroup-long-period-maturity-backtest-result.html 파일이 없습니다. `node vpr-hgroup-long-period-maturity-backtest.js --from=20260101 --to=20260420`를 먼저 실행하세요.");
  }
  res.sendFile(filePath);
});
...
app.get("/pattern", (req, res) => {
  ...
  const qvaStrongCandidates = (result?.qvaStrongCandidates || []).filter(matchSearch);
  ...
  res.render("pattern", {
    ...
    qvaStrongCandidates: qvaStrongCandidates || [],
